function retrieveDailyStatsTumblr() {
  ...
  invocation.onreadystatechange = function () {
    if (tempo == 4) { // jeez christ bad code af dar asigura ca veti primi doar raspunsul la final (json-ul pe care-l asteptati)
      ...
      removeChildren()
      DailyStatstumblr(obj.entries[1].comments_avg, obj.entries[1].likes_avg, obj.entries[1].shares_avg)
    } else { tempo++ }
    if (count == 0) {
      count = 1
...
function DailyStatstumblr(comments_avg, likes_avg, shares_avg) {
...
function retrieveDailyStatsLinkedIn() {
  ...
  invocation.onreadystatechange = function () {
    if (tempo == 4) { // jeez christ bad code af dar asigura ca veti primi doar raspunsul la final (json-ul pe care-l asteptati)
      ...
      var obj = JSON.parse(responseLines[1])
      removeChildren()
      DailyStatslinkedin(obj.entries[1].comments_avg, obj.entries[1].likes_avg, obj.entries[1].shares_avg)
    } else { tempo++ }
    if (count == 0) {
      count = 1
      this.open('GET', url + platforma + actiune + "?date_begin=" + sessionStorage.getItem('date_begin') + "&date_end=" + sessionStorage.getItem('date_end'), true)
...
function DailyStatslinkedin(comments_avg, likes_avg, shares_avg) {
  var elem = document.createElement('div')
  var heading = document.createElement('h4')
  var comments_avg_p = document.createElement('p')
  var likes_avg_p = document.createElement('p')
...
function retrieveDailyStatsTwitter() {
  ...
  invocation.onreadystatechange = function () {
    if (tempo == 4) { // jeez christ bad code af dar asigura ca veti primi doar raspunsul la final (json-ul pe care-l asteptati)
      ...
      var obj = JSON.parse(responseLines[1])
      removeChildren()
      DailyStatstwiiter(obj.entries[1].comments_avg, obj.entries[1].likes_avg, obj.entries[1].shares_avg)
    } else { tempo++ }
    if (count == 0) {
      count = 1
      this.open('GET', url + platforma + actiune + "?date_begin=" + sessionStorage.getItem('date_begin') + "&date_end=" + sessionStorage.getItem('date_end'), true)
...
function DailyStatstwiiter(comments_avg, likes_avg, shares_avg) {
  var elem = document.createElement('div')
  var heading = document.createElement('h4')
  var comments_avg_p = document.createElement('p')
  var likes_avg_p = document.createElement('p')
  ...
  var comments_avg_n = document.createTextNode('comments_avg: ' + comments_avg)
  var likes_avg_n = document.createTextNode('Number of likes_avg: ' + likes_avg)
  var shares_avg_n = document.createTextNode('Number of shares_avg: ' + shares_avg)
